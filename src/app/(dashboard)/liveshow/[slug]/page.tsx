...
import { LiveshowInterface, PodcastInterface } from "@/types/interface";
...
async function getData(slug: string) {
  const query = `*[_type == 'liveshow' && slug.current=='${slug}']{   
      title,
      description,
...
const PodcastStructure = async ({ params }: { params: { slug: string } }) => {
  const liveshow: LiveshowInterface = await getData(params.slug);

  return (
    liveshow.content && (
      <>
        <Navbar />
        <section className="font-jakarta md:py-10 sm:px-[30px] xl:px-[100px]">
          <article className="flex flex-col items-center py-5">
            ...
            {liveshow.imageUrl ? (
              <Image
                src={urlFor(liveshow.imageUrl).url()}
                alt="Blog Post Image"
                width={600}
                height={600}
          ...
          <div className="flex flex-col-reverse md:flex-row prose prose-base sm:prose-xl min-w-full text-gray-400 prose-invert prose-li:marker:text-primary prose-a:text-primary">
            <div className="mx-[10px] md:mx-[0px] rounded-3xl p-4 sm:p-5 md:p-10  bg-slate-100 bg-opacity-5">
              <PortableText
                value={liveshow.content}
                components={myPortableTextComponents}
              />
            </div>
            <Toc headings={liveshow?.headings} />
          </div>
        </section>
      </>
    )
...
export default PodcastStructure;

const Toc = ({ headings }: any) => {
  return (
    <div className="min-w-[30%] md:ml-5">
      <h2 className="text-gray-400 text-xl sm:text-3xl">Table of Contents</h2>
      <nav>
